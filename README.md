...
Open `main_gui.exe`.
Choose language, explanation language, deck, and AI provider.
Generate flashcards or start conversation practice.
Save selected cards to Anki.
Windows warning
...
Recommended flow:
Open the stable GUI.
Click Prepare style for old AI cards.
In Anki Browse, select the shown cards.
Choose Notes → Change Note Type.
Select `AI Vocabulary Light Card · Migrated`.
Map `Front → Front` and `Back → Back`.
